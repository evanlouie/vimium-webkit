...
test("readLogicalLines: strips comments and joins continuations", () => {
...
test('readLogicalLines: `#` and `"` are bindable keys, not trailing comments', () => {
...
test("compileMappings: builds a trie with the canonical notation", () => {
...
test("compileMappings: a prefix and its extension coexist", () => {
...
test("compileMappings: unmap removes an earlier binding", () => {
  const result = compile("map j scrollDown\nunmap j");
  assertEquals(lookup(result.trie, ["j"]), null);
  assertEquals(result.diagnostics.length, 0);
});

test("compileMappings: unmap of nothing warns rather than fails", () => {
  const result = compile("unmap q");
  assertEquals(result.diagnostics[0]?.severity, "warning");
  assertEquals(hasErrors(result), false);
});

test("compileMappings: unmapAll clears everything before it", () => {
  const result = compile("map j scrollDown\nunmapAll\nmap k scrollUp");
  assertEquals(lookup(result.trie, ["j"]), null);
  assertEquals(lookup(result.trie, ["k"])?.binding?.command, "scrollUp");
});

test("compileMappings: options are parsed from the map line", () => {
...
test("compileMappings: an unknown command is an error, not a crash", () => {
  const result = compile("map j scrollDown\nmap k noSuchCommand");
...
test("compileMappings: a malformed key sequence is attributed by line", () => {
  const result = compile("map j scrollDown\nmap <c-a scrollUp");
  const error = result.diagnostics.find((entry) => entry.severity === "error");
  assertEquals(error?.line, 2);
});

test("compileMappings: mapkey records a physical remap", () => {
  const result = compile("mapkey a b");
  assertEquals(result.keyRemap.get("a"), "b");
});

test("compileMappings: mapkey rejects sequences", () => {
  const result = compile("mapkey ab cd");
  assert(hasErrors(result));
});

test("compileMappings: reserved shortcuts are rejected on WebKit", () => {
...
test("compileMappings: reserved shortcuts only warn elsewhere", () => {
...
test("keysByCommand: groups every sequence bound to a command", () => {
  const result = compile("map j scrollDown\nmap <down> scrollDown");
  assertEquals(keysByCommand(result).get("scrollDown"), ["j", "<down>"]);
});

test("DEFAULT_MAPPINGS compiles cleanly against the real registry", () => {
...
test("DEFAULT_MAPPINGS binds Tier C commands too", () => {
...
test("user mappings layer on top of the defaults", () => {
