...
direct call to the backend. The Effect scheduler is a macrotask in a page, so a
value that waits for the fiber is lost when the document goes away. Section 7
says where that path is used.
